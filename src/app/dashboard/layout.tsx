  ...
  admin: [
    ...
    },
    {
      ...
      items: [
        ...
        { href: "/dashboard/admin/verification",  label: "Verifikimi",   icon: <Icon path="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" /> },
    ...
    },
    {
      title: "Sistemi",
      items: [
        { href: "/dashboard/admin/health",    label: "Shëndeti",   icon: <Icon path="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /> },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      // Apply saved platform name + favicon to the browser tab
      void applyBrandToTab();
    };

    void loadUser();
  }, [router]);

  const applyBrandToTab = async () => {
    ...
    if (map["platform_name"]?.trim()) {
      document.title = map["platform_name"].trim();
    }
    if (map["favicon_url"]) {
        ...
        `}>
          ...
          <div className="flex h-16 shrink-0 items-center justify-between gap-2 border-b border-zinc-100 px-5">
            ...
            <div className="flex items-center gap-1">
              {userId && <NotificationBell userId={userId} placement="left" />}
